...
def test_homeostatic_audit_measures_cumulative_drift_not_only_projected_step(monkeypatch):
    monkeypatch.syspath_prepend(str(Path(__file__).parents[1] / "scripts"))
    module = importlib.import_module("audit_credit_updates")
    p = NeuralPerturbationPlasticity(np.array([0, 1]), np.array([2, 2]), np.ones(2), 3,
                                    PlasticityConfig(rule="sensorimotor-perturb-homeostatic-v5",
                                                     input_budget_fraction=.25))
    p.post_baseline[:] = [.2, .4, 0]
    p.eligibility[:] = [-2, 1]
    p.weights[:] = [.25, 1.5]
    assert module.input_drift(p)["maximum_absolute_relative"] > .05
    p.reinforce(1)
    assert module.input_drift(p)["maximum_absolute_relative"] < 1e-7

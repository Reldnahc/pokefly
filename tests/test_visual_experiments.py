...
def test_same_start_reversal_restores_acquired_paired_state_for_both_arms(tmp_path, monkeypatch):
    ...
    write(source / "report.json", {
        "seed": 5, "checkpoints": [32], "reverse_mapping": False,
        "pretest": records, "status": "completed",
        "rows": [{"arm": arm, "training_decisions": 32, "retention": records}
    ...
    for arm, weight in (("paired", 2), ("unpaired_within_cue", 9)):
        ...
        write(source / f"{arm}-training.json", [
    ...
    monkeypatch.setattr(sys, "argv", [
        "probe_visual_curve.py", "--config", str(config), "--seed", "5",
        "--continue-from", str(source), "--allow-reversal", "--same-start-reversal",
    ...
    assert report["pre_reversal_scores"]["paired"] == report["pre_reversal_scores"][
        "unpaired_within_cue"
    ]
...
def test_game_continuation_restores_both_final_modes_without_intro(tmp_path, monkeypatch):
    ...
    write(source / "report.json", {
        "seed": 401, "status": "completed", "config": str(config),
        "config_sha256": module.sha256(config), "rows": rows,
    })
    calls = []

    def checkpoint(path):
        mode = path.parent.name
        return {}, {"directory": str(path.parent / "checkpoint"),
    ...
    monkeypatch.setattr(module, "measure", lambda path: {
        "samples": 18000, "weight_updates_this_evaluation": 0,
        "first_starter": None, "first_party_count": None,
    })
    monkeypatch.setattr(sys, "argv", [
        "evaluate_visual_gameplay.py", "--seed", "401", "--steps", "18000",

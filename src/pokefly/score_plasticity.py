...
or the paper's exact neuron model. Existing excitatory motor/descending inputs
only; no learned readout, action identity, cue label, or RAM input.
"""
...
class LikelihoodPlasticity(SensorimotorPlasticity):
    ...
    def observe(self, fired, dt, *, release, probability, membrane_decay, gain, temperature):
        ...
        if (
            release.shape != (self.n,) or probability.shape != (self.n,)
            or not np.isfinite(release).all() or not np.isfinite(probability).all()
            or ((release < 0) | (release > 1)).any()
            or ((probability < 0) | (probability > 1)).any()
            or not 0 < temperature <= 0.5 or not 0 <= membrane_decay < 1
        ):
        ...
        likelihood_eligibility(
            self.pre, self.post, self.base, release, spike, probability,
            self.membrane_trace, self.eligibility, membrane_decay,
            np.exp(-dt / self.config.eligibility_seconds), gain, temperature,
        )
    ...
    def metrics(self):
        return {
            **super().metrics(),
            "rule": (
                "sensorimotor-preconditioned-likelihood-score-v2"
    ...
    def factor_eligibility(self):
        ...
        return value / self.base if self.config.rule == "sensorimotor-score-v2" else value
